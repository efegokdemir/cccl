...
//! This is a cheap replacement for std::vector
template <class _Tp>
...
private:
  ...
  _CCCL_HOST_DEVICE_API static _Tp* __create(size_t __size)
  {
    ...
    return ::cuda::std::launder(reinterpret_cast<_Tp*>(::cuda::std::__cccl_allocate(__size, alignof(_Tp))));
  }
  ...
  _CCCL_HOST_DEVICE_API void __destroy() noexcept
  {
    if (__begin_ != nullptr)
    {
      ...
      ::cuda::std::__cccl_deallocate(__begin_, __size_, alignof(_Tp));
    }
  }
